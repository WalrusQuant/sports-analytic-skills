#!/usr/bin/env python3
"""Compare constant, logistic, and histogram-GBM models on walk-forward folds."""
...
def main() -> int:
    ...
    if not features or args.min_train_groups < 1:
        raise SystemExit("provide features and set --min-train-groups to at least 1")
    ...
    for index in range(args.min_train_groups, len(groups)):
        ...
        folds.append(
            {
                ...
                "n_test": int(len(test)),
                "constant_log_loss": log_loss(y_test, constant),
                "logistic_log_loss": log_loss(y_test, log_p),
                "hist_gbm_log_loss": log_loss(y_test, gbm_p),
                "logistic_accuracy": float(((log_p >= 0.5) == y_test).mean()),
                "hist_gbm_accuracy": float(((gbm_p >= 0.5) == y_test).mean()),
            }
        )
        ...
        predictions["logistic_probability"] = log_p
        predictions["hist_gbm_probability"] = gbm_p
